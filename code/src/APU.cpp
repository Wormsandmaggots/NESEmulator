...
//
//     if(pulseOut > 1 || pulseOut < 0 || tndOut > 1 || tndOut < 0) {
//         assert(false);
//     }
//
...
APU::APU(Memory *sharedMemory) {
    sharedMemory->beforeWrite.push_back([this](u16 addr, u8 val) -> bool {
        switch (addr) {
            case 0x4000:
                pulse1.writeDutyAndEnvelope(val);
                break;
            case 0x4001:
                pulse1.writeSweep(val);
                break;
            case 0x4002:
                ...
                break;
            case 0x4003:
                pulse1.writeLengthCounterAndTimerHigh(val);
                break;
            case 0x4004:
                pulse2.writeDutyAndEnvelope(val);
                break;
            case 0x4005:
                pulse2.writeSweep(val);
                break;
            case 0x4006:
                ...
                break;
            case 0x4007:
                pulse2.writeLengthCounterAndTimerHigh(val);
                break;
            case 0x4015:
                writeControl(val);
                break;
...
void APU::step(nes_cycle_t count) {
    while(master_cycle < count) {
        ...
        if(sample_cycle.count() > 20) {
            ...
            SDL_LockAudio();
            // len++;
            // sampleBuffer.push_back(len % 256); // Przykładowy sygnał (sawtooth wave
...
std::vector<unsigned char> & APU::getSampleBuffer() {
    return sampleBuffer;
}
...
u8 APU::getSample() {
    double pulseOut = 0.00752 * (pulse1.getSample() + pulse2.getSample());

    return static_cast<uint8_t>(floor(255.0 * (pulseOut)));
}
